...
function bp_dtheme_post_update() {
	...
	if ( empty( $_POST['content'] ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error alert alert-danger"><p>' . __( 'Please enter some content to post.', 'firmasite' ) . '</p></div>' );
	...
	if ( empty( $activity_id ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error alert alert-danger"><p>' . __( 'There was a problem posting your update, please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_dtheme_new_activity_comment() {
	...
	if ( empty( $_POST['content'] ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error alert alert-danger"><p>' . __( 'Please do not leave the comment area blank.', 'firmasite' ) . '</p></div>' );

	if ( empty( $_POST['form_id'] ) || empty( $_POST['comment_id'] ) || ! is_numeric( $_POST['form_id'] ) || ! is_numeric( $_POST['comment_id'] ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error alert alert-danger"><p>' . __( 'There was an error posting that reply, please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_dtheme_delete_activity() {
	...
	if ( ! bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error alert alert-danger"><p>' . __( 'There was a problem when deleting. Please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_dtheme_ajax_addremove_friend() {
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( bp_loggedin_user_id(), $friend_id ) ) {
		...
		if ( ! friends_remove_friend( bp_loggedin_user_id(), $friend_id ) ) {
			...
			echo '<a id="friend-' . esc_attr( $friend_id ) . '" class="add" rel="add" title="' . __( 'Add Friend', 'firmasite' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/add-friend/' . $friend_id, 'friends_add_friend' ) . '">' . __( 'Add Friend', 'firmasite' ) . '</a>';
		}
	...
	// Trying to request friendship
		...
		if ( ! friends_add_friend( bp_loggedin_user_id(), $friend_id ) ) {
			...
			echo '<a id="friend-' . esc_attr( $friend_id ) . '" class="remove" rel="remove" title="' . __( 'Cancel Friendship Request', 'firmasite' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/requests/cancel/' . $friend_id . '/', 'friends_withdraw_friendship' ) . '" class="requested">' . __( 'Cancel Friendship Request', 'firmasite' ) . '</a>';
		}
