...
jq(document).ready( function() {
	...
	/* Textarea focus */
	/*jq('#whats-new_ifr').contents().find('#tinymce').focus( function(){
		jq("#whats-new-options").animate({
			height:'40px'
		});
		jq("form#whats-new-form textarea").animate({
	...
	/*jq('#whats-new_ifr').blur( function(){
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				if ( 0 == jq("ul.activity-list").length ) {
					...
					jq("div.activity").append( '<ul id="activity-stream" class="activity-list item-list list-unstyled">' );
				}
	...
	jq('div.activity').click( function(event) {
		...
		if ( target.parent().hasClass('load-more') ) {
			jq("#primary li.load-more").addClass('loading');
			...
			function(response)
			{
				jq("#primary li.load-more").removeClass('loading');
				jq.cookie( 'bp-activity-oldestpage', oldest_page, {
					path: '/'
				} );
				jq("#primary ul.activity-list").append(response.contents);
					...
					form.fadeOut( 200, function() {
						if ( 0 == activity_comments.children('ul').length ) {
							if ( activity_comments.hasClass('activity-comments') ) {
								activity_comments.prepend('<ul class="list-unstyled"></ul>');
							} else {
								activity_comments.append('<ul class="list-unstyled"></ul>');
							}
						}
	...
	jq('div#primary').click( function(event) {
		var target = jq(event.target);
	...
	jq("a#delete_inbox_messages, a#delete_sentbox_messages").click( function() {
		...
		jq.post( ajaxurl, {
			...
			if ( response[0] + response[1] == "-1" ) {
				...
				jq('#message-threads').before( '<div class="clearfix"></div><div id="message" class="updated alert alert-info"><p>' + response + '</p></div>' );
